...
impl Model {
    ...
    pub fn write_obj(&self, path: &Path) -> Result<()> {
        ...
        let resolved_path = if path.is_absolute() {
            ...
            // Normalize the path to remove any "./" prefixes
            // Don't use canonicalize for output files as they might not exist yet
        ...
        writeln!(file, "# Generated by multi-roof-assignment").map_err(Error::Io)?;
    ...
    fn mark_ground_faces(&mut self) -> Vec<usize> {
        ...
        // Find the minimum Z value across all faces
        ...
        // mark surfaces whose normal is almost horizontal and it's z value is width in 1.0 m
        ...
        for (i, face) in self.faces.iter().enumerate() {
            ...
            if dot_with_up > 1.0 - WALL_ANGLE_THRESHOLD && diff_with_minz < GROUND_HEIGHT_THRESHOLD
            {
    ...
    fn classify_surfaces(&mut self) {
        // First identify ground faces (lowest z value)
        self.mark_ground_faces();
        //count the number of ground faces
        ...
        for i in 0..self.faces.len() {
            if self.faces[i].surface_type != SurfaceType::Unknown {
                continue; // Skip already classified faces
            }
        ...
        //print the faces with their surface type
        for (i, face) in self.faces.iter().enumerate() {
            println!("Face {:?} has surface type {:?}", i, face.surface_type);
        }
    }
    ...
    /// Remove all faces labeled as wall or roof
